function laheta() {
    ...
    const radioSelection = document.querySelector('input[name="parasta"]:checked');
    const checkboxSelections = document.querySelectorAll('input[name="lisaa"]:checked');
    const found = document.getElementById('loytyiko').value;

    const radioText = radioSelection ? radioSelection.value : 'Not selected';
    const checkboxText = Array.from(checkboxSelections).map(checkbox => checkbox.value).join(', ') || 'None';
